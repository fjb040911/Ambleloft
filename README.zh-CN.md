...
**Ambleloft 是免费、开源的通用桌面 Agent 工作台。** 连接自己的模型服务，为 Agent 提供项目上下文，在同一工作区中完成对话、文件浏览、执行审批和结果审阅。

你可以用它理解代码库、整理文档、准备项目交付或探索数据。核心保持通用，专业场景通过技能、扩展及定制集成完成。
...
- **桌面核心：**免费，使用 Apache-2.0 许可证。通过自配兼容模型端点即可工作，无需等待官方托管平台。
- **技能：**可复用的任务指令与本地辅助资源。
- **扩展：**当前为实验性的声明式子集，支持带纯文本视图和导航命令的本地 JSON 清单。尚不执行扩展脚本，也不提供商城。参见[扩展示例与能力边界](examples/extensions/README.md)。
- **商业及定制服务：**托管模型/插件服务和规划中的团队协作与开源桌面核心分离。科研、医学影像等方案在通用工作台基础上合作定制，并非此预览版已内置的专业能力。

## 预览版范围

**当前软件包版本为 0.2.1 Preview。** 项目持续开发中，尚非稳定发行版。目前已验证的桌面目标为 macOS Apple Silicon。Windows、Linux 是目标平台，现有打包流程尚不提供相应发行包。

Ambleloft 连接现有端点，模型下载和推理运行时管理不在当前产品范围内。团队协作、商城安装、可执行扩展和应用连接器尚未实现。实际兼容性取决于模型及服务协议。
