...
import { ChevronRight, Folder, FolderOpen, SquarePen, MoreHorizontal, Plus, MessageCircle, Settings } from 'lucide-react';
...
export default function WorkspaceNavigation({projects,tasks,selected,projectId,open,newTask,openProject,editProject,editTask,addProject}:{projects:Project[];tasks:NavTask[];selected:string|null;projectId:string;open(task:NavTask):void;newTask(project?:string):void;openProject(id:string):void;editProject(project:Project):void;editTask(task:NavTask):void;addProject():void}) {
 ...
 const toggleProject=(id:string)=>{hide();setCollapsed(old=>{const next=new Set(old);if(next.has(id))next.delete(id);else next.add(id);return next;});};
 ...
 const row=(task:NavTask)=><div className={`tree-task ${selected===task.id?'selected':''}`} key={task.id}><button aria-current={selected===task.id?'page':undefined} onClick={()=>open(task)} title={task.title}><span className={`task-state ${task.status}`} aria-label={task.draft?t("草稿"):runStatus[task.status as AgentRun['status']]} /><span>{task.title}</span>{task.unread&&<small aria-label="未读更新">●</small>}{statusLabel(task)&&<small className={`task-status-label ${task.status}`}>{t(statusLabel(task))}</small>}</button><button className="icon-button row-menu" aria-label={`管理任务：${task.title}`} onClick={()=>editTask(task)}><MoreHorizontal size={14}/></button></div>;
 const heading=(key:'projects'|'chats',label:string,action:string,add:()=>void)=><div className="nav-caption caption-action"><button className="nav-group-toggle" aria-expanded={!groups[key]} aria-controls={`nav-${key}`} onClick={()=>{hide();setGroups({...groups,[key]:!groups[key]});}}><span>{t(label)}</span><ChevronRight size={16} style={{transform:groups[key]?undefined:'rotate(90deg)'}}/></button><button className="icon-button nav-group-add" title={t(action)} aria-label={t(action)} onClick={add}>{key==='projects'?<Plus size={18}/>:<SquarePen size={18}/>}</button></div>;
 return <div className="workspace-navigation" onScroll={hide} onKeyDown={e=>{if(e.key==='Escape')hide();}}>
  <section aria-label={t("我的项目")}>{heading('projects','项目','添加项目',addProject)}<div id="nav-projects" className={`nav-disclosure ${groups.projects?'':'is-open'}`} inert={groups.projects} aria-hidden={groups.projects}><div className="nav-disclosure-inner">
  {projects.slice(0,projectLimit).map(project=><VirtualBlock key={project.id} enabled={projects.length>30}><div className="project-tree"><div className={`project-tree-row ${projectId===project.id?'selected':''}`} onClick={()=>{toggleProject(project.id);openProject(project.id);}} onMouseEnter={e=>show(e.currentTarget,project)} onMouseLeave={leave} onContextMenu={e=>{e.preventDefault();hide();editProject(project);}}>
   <button className="icon-button tree-toggle" aria-label={`${collapsed.has(project.id)?t("展开"):t("折叠")}项目：${project.name}`} aria-expanded={!collapsed.has(project.id)}>{collapsed.has(project.id)?<Folder size={18}/>:<FolderOpen size={18}/>}</button>
   <button aria-current={projectId===project.id?'page':undefined} className="project-tree-name" onFocus={e=>show(e.currentTarget,project)} onBlur={leave} onKeyDown={e=>{if(e.key==='ArrowRight'&&preview){e.preventDefault();previewRef.current?.querySelector('button')?.focus();}}} aria-expanded={!collapsed.has(project.id)}><span>{project.name}</span></button>
   <button className="icon-button row-menu" aria-label={`管理项目：${project.name}`} onClick={event=>{event.stopPropagation();hide();editProject(project);}}><MoreHorizontal size={15}/></button>
   <button className="icon-button row-menu" aria-label={`在项目中新建任务：${project.name}`} onClick={event=>{event.stopPropagation();hide();newTask(project.id);}}><Plus size={15}/></button>
  </div><div className={`nav-disclosure ${collapsed.has(project.id)?'':'is-open'}`} inert={collapsed.has(project.id)} aria-hidden={collapsed.has(project.id)}><div className="nav-disclosure-inner"><div className="project-children">{taskRows(project.id)}</div></div></div></div></VirtualBlock>)}{projects.length>projectLimit&&<button className="text-button" onClick={()=>setProjectLimit(n=>n+30)}>{t('加载更多项目')}</button>}
  {!projects.length&&<p className="nav-empty">{t("添加项目，整理相关任务。")}</p>}</div></div></section>
  <section aria-label={t("聊天")}>{heading('chats','聊天','新建聊天',()=>newTask())}<div id="nav-chats" className={`nav-disclosure ${groups.chats?'':'is-open'}`} inert={groups.chats} aria-hidden={groups.chats}><div className="nav-disclosure-inner">{taskRows('chats')}{!tasks.some(t=>!t.projectId)&&<p className="nav-empty">{t("你的聊天会显示在这里。")}</p>}</div></div></section>
