  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      mermaidQueue = mermaidQueue.catch(() => {}).then(async () => {
        ...
        } catch { if (!cancelled) setError('图表尚未完整或语法不受支持，可切换源码查看。'); }
      });
    }, 350);
    return () => { cancelled = true; clearTimeout(timer); };
  }, [source, id]);
  return error ? <p className="artifact-error">{t(error)}</p> : svg ? <img className="diagram-svg" alt={t("Mermaid 图表")} src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`} /> : <p role="status">{t('正在生成图表…')}</p>;
}
export default function Artifact({ language, source }: { language: string; source: string }) {
  ...
  return <section className={`artifact ${expanded ? 'artifact-expanded' : ''}`} aria-label={`${language} 图表`}>
    <div className="artifact-toolbar"><strong>{language === 'drawio' ? t('draw.io · 基础预览') : language}</strong><div>
      ...
      <button type="button" onClick={() => setExpanded(!expanded)}>{expanded ? t('收起') : t('放大')}</button>
