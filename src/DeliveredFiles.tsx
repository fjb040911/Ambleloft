...
import {t} from './i18n';
const OfficeArtifact=lazy(()=>import('./OfficeArtifactPreview'));
export default function DeliveredFiles({runId,files,openFile}:{runId:string;files:DeliveredFile[];openFile?(file:DeliveredFile):void}) {
 ...
 const [office,setOffice]=useState<DeliveredFile|null>(null);
 ...
 return <div className="delivered-files" aria-label={t('交付文件')}>{files.map(file=><div className="delivered-file" key={file.id}><FileTypeIcon name={file.name}/><div title={file.path}><button type="button" className="delivered-file-name" disabled={!!busy} onClick={()=>openFile?openFile(file):void access(file)}>{file.name}</button><small>{file.size<1024?`${file.size} B`:`${(file.size/1024).toFixed(1)} KB`}</small></div>{(/\.(xlsx?|pptx?)$/i.test(file.name)&&file.size<=32*1024*1024)&&<button className="icon-button" aria-label={`${t('预览')} ${file.name}`} disabled={!!busy} onClick={()=>openFile?openFile(file):setOffice(file)}><Eye size={17}/></button>}{(previewPolicy.extensions.includes(file.name.split('.').pop()?.toLowerCase()||'')&&file.size<=previewPolicy.maxBytes)&&<button className="icon-button" aria-label={`${t('预览')} ${file.name}`} disabled={!!busy} onClick={()=>void access(file,true)}><Eye size={17}/></button>}<button className="icon-button" aria-label={`${t('在访达中显示')} ${file.name}`} disabled={!!busy} onClick={()=>void access(file)}><FolderOpen size={17}/></button></div>)}{error&&<p role="alert" className="error-banner">{error}</p>}{office&&<Suspense fallback={null}><OfficeArtifact artifactId={office.id} runId={runId} path={office.path} name={office.name} close={()=>setOffice(null)}/></Suspense>}{preview&&<ArtifactPreview name={preview.name} text={preview.text} close={()=>setPreview(null)}/>}</div>;
}
