...
import { capabilities, models } from './catalog';
import Modal from './Modal';
import {ModelCatalogPage,CapabilityCatalogPage,initialModelView,initialCapabilityView,type ModelView} from './CatalogPages';
import {useCatalog} from './catalog-service';
...
import type { AgentRun, ProviderConfig, ProviderCatalog, Capability, Device, Model, Page, Project, Task, Theme, Workspace } from './types';

const pages = [{ id: 'models', label: '模型中心', icon: Cpu }, { id: 'plugins', label: '能力与插件', icon: Puzzle }] as const;
const capabilityIcons = { folder: Folder, pen: PencilLine, research: BookOpen };


export default function App() {
  useLanguage();
  const directory=useCatalog();
  const [modelView,setModelView]=useState(initialModelView);
  const [capabilityView,setCapabilityView]=useState(initialCapabilityView);
  const [settingsModelView,setSettingsModelView]=useState<ModelView>(()=>({...initialModelView,filter:'local' as const}));
  const [settingsCapabilityView,setSettingsCapabilityView]=useState(initialCapabilityView);
  ...
  const pageLabel = page === 'settings' ? t("设置") : page==='home' ? (currentRun?currentRun.title:t("新建任务")) : page==='projects'?t("我的项目"):page==='devices'?t("我的设备"):pages.find(item => item.id === page)?.label;
  ...
  return <><div inert={settingsOpen} style={settingsOpen?{visibility:'hidden',pointerEvents:'none'}:undefined} className={`app-shell ${sidebar ? '' : 'sidebar-hidden'} ${window.desktop ? 'desktop' : 'browser'}`}>
    <aside id="workspace-sidebar" className="sidebar" inert={!sidebar} aria-label={t("主导航")}>
      ...
      <nav>{pages.map(item => <button key={item.id} className={`nav-item ${page === item.id ? 'active' : ''}`} onClick={() => navigate(item.id)} aria-current={page === item.id ? 'page' : undefined}><item.icon size={18} strokeWidth={1.65} /><span>{t(item.label)}</span>{item.id === 'models' && <span className="nav-tag">{t("探索")}</span>}</button>)}</nav>
      ...
      <div className="sidebar-bottom"><button className="local-status" onClick={() => navigate('devices')}><span className="device-mini"><Laptop size={18} /></span><span>{device?.mode === 'desktop' ? t("本机已连接") : t("本地优先")}<small>{device?.memoryGB ? `${device.memoryGB} GB ${t('内存')}` : t("资料由你掌控")}</small></span><span className="status-dot" /></button><div className="sidebar-footer"><button className={`nav-item ${page === 'settings' ? 'active' : ''}`} onClick={() => navigate('settings')}><Settings size={17} />{t("设置")}</button><button className="icon-button" onClick={() => setModal('help')} aria-label={t("关于此版本")}><CircleHelp size={17} /></button></div></div>
    </aside>
    <div className="main-shell"><div className="workspace-content"><div className="conversation-shell">
      ...
      <main id="main-content" className={page === 'home' && currentRun ? 'chat-main' : undefined}>
        ...
        {page === 'home' && !currentRun && <div className="home-page">
          ...
          <div className="composer-note"><ShieldCheck size={13} /><span className="composer-status">{provider?.configured ? `模型服务： ${selectedProvider?.baseUrl} · ${composerOptions.permission==='full'?'完全访问':'默认权限'}` : `${selected.name} · ${t("草稿偏好")} · ${t("配置模型服务即可开始；未配置时只保存草稿")}`}</span>{!provider?.configured && <button className="text-button" onClick={() => navigate('settings')}>{t("配置服务")}</button>}<span className="keyboard-hint">↵ {provider?.configured ? t("发送") : t("保存草稿")}</span></div>
        ...
        {page === 'models' && <ModelCatalogPage directory={directory} view={modelView} setView={setModelView} choose={model=>{newTask();setSelectedModel(model.id);}} />}
        {page === 'plugins' && <CapabilityCatalogPage directory={directory} view={capabilityView} setView={setCapabilityView} useTemplate={useCapability} />}
