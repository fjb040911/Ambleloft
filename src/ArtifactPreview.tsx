...
export default function ArtifactPreview({name,text,close}:{name:string;text:string;close():void}) {
 ...
 const [maximized,setMaximized]=useState(false),[zoom,setZoom]=useState(100);
 ...
 return <dialog ref={dialog} className={`modal delivered-preview-dialog${maximized?' is-maximized':''}`} aria-labelledby={titleId} onCancel={event=>{event.preventDefault();close();}} onClick={event=>{
  ...
  <header className="artifact-preview-heading">
   <FileTypeIcon name={name}/><h2 id={titleId} title={name}>{name}</h2>
   <div className="artifact-preview-toolbar" role="toolbar" aria-label={t('文件预览工具栏')}>
    ...
    <button className="icon-button" aria-label={t(maximized?'退出最大化':'最大化')} title={t(maximized?'退出最大化':'最大化')} onClick={toggleMaximized}>{maximized?<Minimize2 size={18}/>:<Maximize2 size={18}/>}</button>
    <button className="icon-button" aria-label={t('关闭')} title={t('关闭')} onClick={close}><X size={18}/></button>
   </div>
  </header>
  <div className={`artifact-preview-body ${markdown?'is-markdown':'is-code'}`}>
