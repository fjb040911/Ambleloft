...
function createProjectFiles({getWorkspace,getRuns,getApprovedApps=async()=>[],setApprovedApps=async()=>{}}) {
 const approvedApps=new Set();
 const contexts = new Map();
 async function rootFor(input) {
  if(!input||typeof input.projectId!=='string')throw new Error('请选择项目');
  const project=(await getWorkspace()).projects.find(p=>p.id===input.projectId);
  if(!project)throw new Error('项目不存在');
  const run=input.runId?(await getRuns()).find(r=>r.id===input.runId):null;
  if(input.runId&&(!run||run.projectId!==project.id))throw new Error('会话与项目不匹配');
 ...
 async function context(input) {
  ...
  const key=JSON.stringify([input.projectId,input.runId||'',root]);
  let token;
  for(const [id,value] of contexts)if(value.key===key)token=id;
  if(!token){token=crypto.randomBytes(24).toString('hex');contexts.set(token,{key,root,input:{projectId:input.projectId,runId:input.runId}});}
 ...
 async function read(input) {
  ...
  const type=['md','markdown'].includes(ext)?'markdown':['html','htm'].includes(ext)?'html':mime[ext]?.startsWith('image/')?'image':ext==='pdf'?'pdf':'code';
  const base={path:relative,name:path.basename(relative),size:stat.size,version,kind:type,truncated:false};
  if(['image','pdf'].includes(type))return {...base,kind:stat.size>MEDIA_LIMIT?'unsupported':type,reason:stat.size>MEDIA_LIMIT?'文件超过 32 MiB，请使用外部应用打开。':''};
 ...
 return {context,list,search,read,resolve,resource,apps,open};
}
