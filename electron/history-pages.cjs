...
function pageRun(run,before,limit=30) {
 ...
 return {...run,messages,fileChanges:run.fileChanges?.filter(p=>keys.has(p.turnKey)),tools:run.tools.filter(t=>keys.has(t.turnKey)||(!t.turnKey&&users.length===1)),plans:run.plans?.filter(p=>keys.has(p.turnKey)),artifacts:run.artifacts?.filter(p=>keys.has(p.turnKey)),summaryOnly:false,historyBefore:start?first.id:null,turnOffset:start,totalTurns:users.length};
}
